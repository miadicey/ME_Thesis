...
def gpg(time, n):
    ...
    for i in range(1, (n2+3)):
        points_v[i-1] = A*np.sin(OMEGA-time)
        points_h[i-1] = -A*np.sin(OMEGA-time+np.pi/2)# + (2*np.pi)/6
    ...
    for k in range(0, n2):
        j = 2*k
        desired_pos[j] = points_v[k]#np.arctan(m_v[k+1]-m_v[k])/(1 + m_v[k+1]*m_v[k])
        desired_pos[j+1] = points_h[k]#np.arctan(m_h[k+1]-m_h[k])/(1 + m_h[k+1]*m_h[k])
    ...
    while (1):
        ...
        try:
            ...
            if (current_time - prev_command_time) >= COMMAND_PERIOD:                        # check if command period has passed since last command                                       
                ...
                angs = np.rad2deg(gpg(current_time, N_JOINTS))
                print(angs)
